...
export default class Menu extends Base {
  ...
  capitalize(str) {
    return (str.substring(0, 1).toUpperCase() + str.substring(1)).replace(/([a-z]{1})([A-Z]{1})/g, '$1 $2')
  }
      ...
      <div className={`menu ${this.Store.menuVisibility ? 'show' : ''}`} onClick={this.updateState}>
        ...
        {rows}
        {
          this.Store.isAdminMode
            ? <div className={'newSection'}>Add a new section
              <div>
                <input
                  name="section" placeholder="Section" id="section"
                  onChange={this.handleSection}
                  style={{width: 100}}/>{' '}
                <button type={'button'} disabled={!section} onClick={this.handleSubmit}>Add</button>
              </div>
            </div>
            : null
        }
